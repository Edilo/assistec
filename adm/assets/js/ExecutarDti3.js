...
//    alert(atoccoddti);
    ...
    if (atteccod1dti === 'Selecione') {
        alert('Obrigatório escolher pelo menos um técnico!');
    } else {
        $.ajax({
            type: "POST",
...
$("#finalizarDti").click(function () {
    ...
    if (atteccod1dti === "Selecione") {
        alert("Obrigatório escolher pelo menos um técnico!");
    } else if (atdescricaodti.length < 10){
        alert("A descrição da atividade não pode ser vazia e tem que ser bem explicada!");
    }else{
